                ...
                <table class="table table-hover table-striped">
                    ...
                    <tbody>
                        @forelse($requests as $request)
                            ...
                            @endphp
                            <tr>
                                <td>{{ $request->request_number ?? 'MR-'.str_pad($request->id, 4, '0', STR_PAD_LEFT) }}</td>
                                <td>{{ $request->request_name }}</td>
                                <td>{{ $request->request_date ? \Carbon\Carbon::parse($request->request_date)->format('Y-m-d') : 'N/A' }}</td>
                                <td><span class="badge bg-{{ $priorityColor }}">{{ $request->priority ?? 'Normal' }}</span></td>
                                <td><span class="badge bg-{{ $statusColor }}">{{ ucfirst($request->status) }}</span></td>
